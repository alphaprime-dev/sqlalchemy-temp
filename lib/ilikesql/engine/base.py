...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    ...
    _sqla_logger_namespace = "ilikesql.engine.Connection"

    # used by ilikesql.engine.util.TransactionalContext
    ...
    def execution_options(self, **opt: Any) -> Connection:
        ...
        The keywords that are currently recognized by ilikesql itself
        ...
        :param logging_token: Available on: :class:`_engine.Connection`,
          ...
          ``logging.getLogger("ilikesql.engine")`` logger. This allows a
    ...
    def connection(self) -> PoolProxiedConnection:
        ...
        This is a ilikesql connection-pool proxied connection
    ...
    def invalidate(self, exception: Optional[BaseException] = None) -> None:
        ...
        :class:`~ilikesql.exc.InvalidRequestError`.
    ...
    def begin(self) -> RootTransaction:
        ...
        if self._transaction is None:
            ...
            raise exc.InvalidRequestError(
                "This connection has already initialized a ilikesql "
    ...
    def _handle_dbapi_exception(
        ...
        try:
            ...
            if should_wrap:
                ilikesql_exception = exc.DBAPIError.instance(
                    statement,
                    parameters,
                    cast(Exception, e),
                    self.dialect.loaded_dbapi.Error,
            ...
            if (self.dialect._has_events) and not self._execution_options.get(
                ...
                if self._is_disconnect != ctx.is_disconnect:
                    ...
                    if ilikesql_exception:
                        ilikesql_exception.connection_invalidated = (
                            ctx.is_disconnect
                        )
            ...
            if newraise:
                ...
                assert ilikesql_exception is not None
                raise ilikesql_exception.with_traceback(exc_info[2]) from e
            else:
    ...
    def _handle_dbapi_exception_noconnection(
        ...
        if dialect._has_events:
            ...
            if ilikesql_exception and is_disconnect != ctx.is_disconnect:
                ilikesql_exception.connection_invalidated = (
                    is_disconnect
                ) = ctx.is_disconnect
...
class ExceptionContextImpl(ExceptionContext):
    ...
    def __init__(
        ...
        ilikesql_exception: Optional[exc.StatementError],
        ...
        self.ilikesql_exception = ilikesql_exception
...
class Engine(
    ...
    """
    Connects a :class:`~ilikesql.pool.Pool` and
    :class:`~ilikesql.engine.interfaces.Dialect` together to provide a
    source of database connectivity and behavior.

    An :class:`_engine.Engine` object is instantiated publicly using the
    :func:`~ilikesql.create_engine` function.
    ...
    _sqla_logger_namespace: str = "ilikesql.engine.Engine"
    ...
    @property
    def name(self) -> str:
        """String name of the :class:`~ilikesql.engine.interfaces.Dialect`
    ...
    @property
    def driver(self) -> str:
        """Driver name of the :class:`~ilikesql.engine.interfaces.Dialect`
        in use by this :class:`Engine`.
