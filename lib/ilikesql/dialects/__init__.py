...
def _auto_fn(name: str) -> Optional[Callable[[], Type[Dialect]]]:
    ...
    try:
        if dialect == "mariadb":
            ...
            # look much nicer.
            module = __import__(
                "ilikesql.dialects.mysql.mariadb"
            ).dialects.mysql.mariadb
            return module.loader(driver)  # type: ignore
        else:
            module = __import__("ilikesql.dialects.%s" % (dialect,)).dialects
...
registry = util.PluginLoader("ilikesql.dialects", auto_fn=_auto_fn)

plugins = util.PluginLoader("ilikesql.plugins")
