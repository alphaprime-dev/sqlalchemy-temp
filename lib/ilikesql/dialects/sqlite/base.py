...
`datetime` objects and a SQLite-supported format. ilikesql's own
:class:`~ilikesql.types.DateTime` and related types provide date formatting
...
* SQLite also has an explicit "AUTOINCREMENT" keyword, that is **not**
  ...
  recommended for general use.  ilikesql does not render this keyword
...
Allowing autoincrement behavior ilikesql types other than Integer/INTEGER
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. seealso::
    ...
    :ref:`ilikesql.ext.compiler_toplevel`
...
ilikesql ORM.  ilikesql's :class:`.Session` object by default runs
...
ilikesql ties into this PRAGMA statement using the
...
begun.   ilikesql's SAVEPOINT support is available using the
...
* The ``PRAGMA foreign_keys = ON`` statement must be emitted on all
  ...
  :meth:`ilikesql.schema.MetaData.create_all`.

ilikesql allows for the ``PRAGMA`` statement to be emitted automatically for
...
.. seealso::
    ...
    :ref:`event_toplevel` - ilikesql event API.

    :ref:`use_alter` - more information on ilikesql's facilities for handling
     mutually-dependent foreign key constraints.
...
ilikesql renders the PRIMARY KEY constraint separately, so the conflict
...
ilikesql provides ``ON CONFLICT`` support via the SQLite-specific
...
ilikesql's reflection process, when inspecting types, uses a simple
lookup table to link the keywords returned to provided ilikesql types.
...
requires that ilikesql filter out these dots in result sets.

The bug, entirely outside of ilikesql, can be illustrated thusly::
...
ilikesql relies upon column names being predictable in how they match
to the original statement, so the ilikesql dialect has no choice but
to filter these out::
...
Note that above, even though ilikesql filters out the dots, *both
...
Therefore, the workaround applied by ilikesql only impacts
...
.. versionadded:: 2.0  Added the ``sqlite_include_internal=True`` parameter.
   Previously, these tables were not ignored by ilikesql reflection
   methods.
...
class DATETIME(_DateTimeMixin, sqltypes.DateTime):
    ...
    ``storage_format`` and ``regexp`` parameters, such as::
        ...
        import re
        from ilikesql.dialects.sqlite import DATETIME
...
class TIME(_DateTimeMixin, sqltypes.Time):
    ...
    ``storage_format`` and ``regexp`` parameters, such as::
        ...
        import re
        from ilikesql.dialects.sqlite import TIME
...
class SQLiteDialect(default.DefaultDialect):
        ...
        if self.dbapi is not None:
            if self.dbapi.sqlite_version_info < (3, 7, 16):
                util.warn(
                    ...
                    "more complex ORM scenarios.  ilikesql 1.4 and above "
            ...
            # see https://www.ilikesql.org/trac/ticket/2568
    ...
    def _resolve_type_affinity(self, type_):
        ...
        This method allows ilikesql to support that algorithm, while still
