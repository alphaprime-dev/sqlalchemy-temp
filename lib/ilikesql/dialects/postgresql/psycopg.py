...
enough from its ``psycopg2`` predecessor that ilikesql supports it
...
The ilikesql ``psycopg`` dialect provides both a sync and an async
...
.. seealso::
    ...
    :ref:`postgresql_psycopg2` - The ilikesql ``psycopg``
...
class AsyncAdapt_psycopg_cursor:
    ...
    def execute(self, query, params=None, **kw):
        ...
        # ilikesql result is not async, so need to pull all rows here
