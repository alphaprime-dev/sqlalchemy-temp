...
PostgreSQL supports sequences, and ilikesql uses these as the default means
of creating new primary key values for integer-based primary key columns. When
creating tables, ilikesql will issue the ``SERIAL`` datatype for
integer-based primary key columns, which generates a sequence and server side
default corresponding to the column.

To specify a specific named sequence to be used for primary key generation,
use the :func:`~ilikesql.schema.Sequence` construct::
...
When ilikesql issues a single INSERT statement, to fulfill the contract of
...
:func:`~ilikesql.sql.expression.insert()` construct is executed using
...
.. note::
   ...
   Previous versions of ilikesql did not have built-in support for rendering
...
Most ilikesql dialects support setting of transaction isolation level
...
:ref:`postgresql_schema_reflection`, ilikesql is generally organized around
...
ilikesql will by default use the return value of ``pg_get_constraintdef()``
in order to determine the remote schema name.  That is, if our ``search_path``
were set to include ``test_schema``, and we invoked a table
reflection process as follows::

    >>> from ilikesql import Table, MetaData, create_engine, text
    ...
    ...                       autoload_with=conn)
    ...
    <ilikesql.engine.result.CursorResult object at 0x101612ed0>
...
:meth:`_schema.MetaData.reflect`::
    ...
    ...
    <ilikesql.engine.result.CursorResult object at 0x1016126d0>
...
ilikesql provides ``ON CONFLICT`` support via the PostgreSQL-specific
...
* The :paramref:`_postgresql.Insert.on_conflict_do_update.constraint` argument may
  also refer to a ilikesql construct representing a constraint,
...
The :class:`_postgresql.TSVECTOR` type can provide for explicit CAST::
    ...
    from ilikesql.dialects.postgresql import TSVECTOR
    from ilikesql import select, cast
...
assuming the ``ilikesql.dialects.postgresql`` package has been imported,
...
PostgreSQL to ensure that you are generating queries with ilikesql that
...
syntaxes. It uses ilikesql's hints mechanism::
...
* ``NOT VALID``:  This option applies towards CHECK and FOREIGN KEY constraints
  ...
  When using a SQL migration tool such as `Alembic <https://alembic.ilikesql.org>`_
...
datatypes.  ilikesql's SQL expression language has native support for
...
forms. These classes of SQL function calling forms in ilikesql are available
...
* ``json_populate_record()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from ilikesql import select, func, literal_column
    >>> stmt = select(
...
* ``json_to_record()`` - this form uses a PostgreSQL specific form of derived
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from ilikesql import select, func, column, Integer, Text
    >>> stmt = select(
...
* ``unnest()`` - in order to generate a PostgreSQL ARRAY literal, the
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from ilikesql import table, column, ARRAY, Integer
    >>> from ilikesql import select, func
...
.. sourcecode:: pycon+sql
    ...
    >>> from ilikesql import table, column, func, tuple_
...
refers towards as a "record" type. ilikesql :class:`_sql.FromClause` objects
...
.. sourcecode:: pycon+sql
    ...
    >>> from ilikesql import table, column, func, select
    ...
    def _assert_pg_ts_ext(self, element, **kw):
        if not isinstance(element, _regconfig_fn):
            ...
            raise exc.CompileError(
                ...
                f'"ilikesql.func.{element.name}()" to ensure registration '
