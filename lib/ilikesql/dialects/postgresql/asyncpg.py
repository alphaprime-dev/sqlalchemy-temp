...
# Copyright (C) 2005-2023 the ilikesql authors and contributors <see AUTHORS
# file>
#
...
The asyncpg dialect is ilikesql's first Python asyncio dialect.

Using a special asyncio mediation layer, the asyncpg dialect is usable
as the backend for the :ref:`ilikesql asyncio <asyncio_toplevel>`
extension package.
...
adding the ilikesql-specific flag ``async_fallback`` to the URL
...
.. note::
    ...
    returns them as strings. ilikesql sets default type decoder for ``json``
...
The asyncpg ilikesql dialect makes use of ``asyncpg.connection.prepare()``
...
is implemented by ilikesql, it is part of the DBAPI emulation portion of the
...
.. warning::  The ``asyncpg`` database driver necessarily uses caches for
   ...
   prepared statements themselves which are optionally cached by ilikesql's
   ...
   The ilikesql asyncpg dialect will invalidate these caches within its local
   ...
   operated upon the previous structures. The ilikesql asyncpg dialect will
...
.. seealso::
   ...
   https://github.com/ilikesql/ilikesql/issues/6467
...
class AsyncAdapt_asyncpg_dbapi:
    ...
    class InvalidCachedStatementError(NotSupportedError):
        def __init__(self, message):
            super().__init__(
                message + " (ilikesql asyncpg dialect will now invalidate "
                "all prepared caches in response to this exception)",
            )

    # pep-249 datatype placeholders.  As of ilikesql 2.0 these aren't
