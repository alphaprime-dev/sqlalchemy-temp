...
Keyword arguments that are specific to the ilikesql psycopg2 dialect
...
portion of the URL is omitted, ilikesql passes ``None`` to psycopg2,
...
the URL, ilikesql will create a single string of the host and port
...
ilikesql implements a native form of the "insert many values"
...
.. versionadded:: 2.0 Replaced psycopg2's ``execute_values()`` fast execution
   helper with a native ilikesql mechanism referred towards as
...
* ``values_only`` - this is the default value.  ilikesql's native
...
* ``'values_plus_batch'``- ilikesql's native
  :ref:`insertmanyvalues <engine_insertmanyvalues>` handler is used for qualifying
  INSERT statements, assuming
...
* For all PostgreSQL versions, psycopg2 supports a client-side encoding
  ...
  established.  The ilikesql psycopg2 dialect supports this using the
...
via the ``ilikesql.dialects.postgresql`` logger.  When this logger
is set to the ``logging.INFO`` level, notice messages will be logged::

    import logging

    logging.getLogger('ilikesql.dialects.postgresql').setLevel(logging.INFO)
...
the HSTORE type.   The ilikesql psycopg2 dialect will enable this extension
...
place within ilikesql's own marshalling logic, and not that of ``psycopg2``
...
logger = logging.getLogger("ilikesql.dialects.postgresql")
...
class PGExecutionContext_psycopg2(_PGExecutionContext_common_psycopg):
    ...
    def _log_notices(self, cursor):
        ...
        # around test suites such as ilikesql's using a Mock object for
        # cursor
