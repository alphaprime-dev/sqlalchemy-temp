...
then connect in ilikesql using the ``service_name`` query string parameter::
...
Options consumed by the ilikesql cx_Oracle dialect outside of the driver
--------------------------------------------------------------------------

There are also options that are consumed by the ilikesql cx_oracle dialect
...
be used in place of ilikesql's pooling functionality.  This can be achieved
...
ilikesql dialect to use NCHAR/NCLOB for the :class:`.Unicode` /
...
``Cursor.var()``, as well as ilikesql's own decoding function, as the
...
possible for ilikesql to know exactly how data should be bound, as some
...
On the ilikesql side, the :meth:`.DialectEvents.do_setinputsizes` event can
...
ilikesql perspective before they are converted to the raw ``setinputsizes()``
...
objects which have a ``.key`` and a ``.type`` attribute::
    ...
    def _log_setinputsizes(inputsizes, cursor, statement, parameters, context):
        for bindparam, dbapitype in inputsizes.items():
                log.info(
                    "Bound parameter name: %s  ilikesql type: %r  "
                    "DBAPI object: %s",
...
however is set by default for the ``Text`` type within the ilikesql 1.2
...
datatypes to be delivered as a single buffer. As such, ilikesql makes use of
...
to support this system is not implemented in ilikesql.
...
ilikesql's numeric types can handle receiving and returning values as Python
...
use, as well as if no ilikesql typing objects are present.   There are
...
ilikesql type (or a subclass of such).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if dbapi_module is not None:
            # these constants will first be seen in ilikesql datatypes
