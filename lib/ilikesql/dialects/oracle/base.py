...
ilikesql Table objects which include integer primary keys are usually
...
feature, ilikesql relies upon sequences to produce these values.   With the
...
specify sequences, use the ilikesql.schema.Sequence object which is passed
...
names using UPPERCASE text.   ilikesql on the other hand considers an
...
an UPPERCASE name on the ilikesql side indicates a case sensitive
identifier, and ilikesql will quote the name - this will cause mismatches
...
lowercase names should be used on the ilikesql side.
...
greater it is now 128.   This change impacts ilikesql in the area of
...
name = 'compatible';``.   The ilikesql Oracle dialect, when tasked with
...
As of ilikesql 1.4, the default max identifier length for the Oracle dialect
...
ilikesql to change this default conservatively.   For example, the following
...
Applications which have run versions of ilikesql prior to 1.4 on an  Oracle
...
* ilikesql currently makes use of ROWNUM to achieve
...
(that is, a ``cursor.execute()`` style statement; ilikesql does not generally
...
When using the ilikesql ORM, the ORM has limited ability to manually issue
...
.. warning:: The status of Oracle 8 compatibility is not known for ilikesql
   2.0.
...
* the NVARCHAR2 and NCLOB datatypes are no longer generated as DDL when
  the :class:`~ilikesql.types.Unicode` is used - VARCHAR2 and CLOB are issued
  instead. This because these types don't seem to work correctly on Oracle 8
  even though they are available. The :class:`~ilikesql.types.NVARCHAR` and
  :class:`~ilikesql.dialects.oracle.NCLOB` types will always generate
  NVARCHAR2 and NCLOB.
...
Bitmap indexes cannot be unique and cannot be compressed. ilikesql will not
