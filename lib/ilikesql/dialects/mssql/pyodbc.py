...
database using Azure credentials::
    ...
    def provide_token(dialect, conn_rec, cargs, cparams):
        # remove the "Trusted_Connection" parameter that ilikesql adds
...
.. tip::
    ...
    The ``Trusted_Connection`` token is currently added by the ilikesql
...
and that can cause problems with ilikesql's "autobegin" (and implicit
...
ilikesql itself.  As ilikesql has its own pooling behavior, it is often
...
ilikesql engine itself fully discards a connection or if the engine is
disposed.
...
Previous limitations with the ilikesql ORM's "versioned rows" feature with
Pyodbc have been resolved as of ilikesql 2.0.5. See the notes at
...
cursor when an executemany call is to be used.   The ilikesql PyODBC SQL
...
.. versionchanged:: 2.0.9 - the ``fast_executemany`` parameter now has its
   ...
   include RETURNING.  Previously, ilikesql 2.0's :term:`insertmanyvalues`
