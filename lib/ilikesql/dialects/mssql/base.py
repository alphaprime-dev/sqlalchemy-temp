...
In addition to the above DBAPI layers with native ilikesql support, there
...
table. ilikesql considers ``IDENTITY`` within its default "autoincrement"
...
:class:`.Sequence` - and will generate DDL as such::
    ...
    from ilikesql import Table, MetaData, Column, Integer
...
.. note::
    ...
    ``autoincrement=True`` to enable the IDENTITY keyword, ilikesql does not
...
.. note::
    ...
    enabled.   The ilikesql SQL Server dialect will perform this operation
...
parameters passed to the :class:`_schema.Identity` object::
    ...
    from ilikesql import Table, Integer, Column, Identity
...
.. versionchanged:: 1.3.19  The :class:`_schema.Identity` object is
   ...
   functional in the normal way starting from ilikesql version 1.4.
...
implement this pattern smoothly in ilikesql, the primary datatype of the
...
for a given ``IDENTITY`` column, a process which ilikesql performs
...
* In the vast majority of cases, RETURNING is used in conjunction with INSERT
  ...
  As of ilikesql 2.0, the :ref:`engine_insertmanyvalues` feature is also
  ...
  .. versionchanged:: 2.0.10 The :ref:`engine_insertmanyvalues` feature for
     SQL Server was temporarily disabled for ilikesql version 2.0.9 due to
...
that refers to the identity column explicitly.  The ilikesql dialect will
...
"OFFSET n ROWS" and "FETCH NEXT n ROWS" clauses.  ilikesql supports these
...
by the ilikesql :class:`.Engine` object includes
...
Server in a future release.   ilikesql normally relates these types to the
...
* Complete control over whether the "old" or "new" types are rendered is
  available in all ilikesql versions by using the UPPERCASE type objects
...
a backwards compatibility mode ilikesql may attempt to use T-SQL
...
ilikesql by default uses OUTPUT INSERTED to get at newly
...
OUTPUT INSERTED is used.    Previous versions of ilikesql therefore had
...
ilikesql 2.0 now retrieves the "rowcount" manually for these particular use
...
impacted by it. As of ilikesql 2.0.5, ORM versioning has been fully
...
class MSTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_JSON(self, type_, **kw):
        # this is a bit of a break with ilikesql's convention of
