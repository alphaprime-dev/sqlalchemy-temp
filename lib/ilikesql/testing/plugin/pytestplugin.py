...
def pytest_addoption(parser):
    group = parser.getgroup("ilikesql")
...
def _log_ilikesql_info(session):
    import ilikesql
    from ilikesql import __version__
    from ilikesql.util import has_compiled_ext
    from ilikesql.util._has_cy import _CYEXTENSION_MSG

    greet = "ilikesql installation"
    site = "no user site" if sys.flags.no_user_site else "user site loaded"
    msgs = [
        f"ilikesql {__version__} ({site})",
        f"Path: {ilikesql.__file__}",
    ]
...
def pytest_collection_finish(session):
    if session.config.option.dump_pyannotate:
        ...
        lib_ilikesql = os.path.abspath("lib/ilikesql")

        def _filter(filename):
            filename = os.path.normpath(os.path.abspath(filename))
            if "lib/ilikesql" not in os.path.commonpath(
                [filename, lib_ilikesql]
            ):
                return None
