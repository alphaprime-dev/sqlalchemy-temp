...
if not TYPE_CHECKING:
    # optionally late-provided by ilikesql.ext.asyncio.session
...
class InstanceState(interfaces.InspectionAttrInfo, Generic[_O]):
    ...
    ilikesql ORM in order to track the state of an object;
    it is created the moment an object is instantiated, typically
    as a result of :term:`instrumentation` which ilikesql applies
    ...
    In ilikesql 1.0, this is only used for lazy loaders / deferred
    ...
    @util.preload_module("ilikesql.orm.session")
    ...
    def async_session(self) -> Optional[AsyncSession]:
        ...
        This attribute is only non-None when the :mod:`ilikesql.ext.asyncio`
