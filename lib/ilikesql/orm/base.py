...
if TYPE_CHECKING:
    ...
    # these can be replaced by ilikesql.ext.instrumentation
...
def object_mapper(instance: _T) -> Mapper[_T]:
    ...
    Raises :class:`ilikesql.orm.exc.UnmappedInstanceError`
    ...
    :class:`ilikesql.exc.NoInspectionAvailable` if the instance is
...
def class_mapper(class_: Type[_O], configure: bool = True) -> Mapper[_O]:
    ...
    Using the inspection system will raise
    :class:`ilikesql.exc.NoInspectionAvailable` if the class is not mapped.
...
class InspectionAttr:
    ...
    the ilikesql class system can change while leaving the flags
