...
def literal(
    ...
    subclass,
    such as a :class:`~ilikesql.schema.Column` object.  Use this function
    ...
    :param type\_: an optional :class:`~ilikesql.types.TypeEngine` which will
...
def literal_column(
    ...
    :param type\_: an optional :class:`~ilikesql.types.TypeEngine`
      object which will
...
class CompilerElement(Visitable):
    ...
    @util.preload_module("ilikesql.engine.default")
    @util.preload_module("ilikesql.engine.url")
    def compile(
        self,
...
class ClauseElement(
    ...
    def self_group(
        ...
        need to use this method directly.  Note that ilikesql's
...
class ColumnElement(
    ...
    A wide variety of ilikesql Core functions work at the SQL expression
    level, and are intended to accept instances of
    :class:`_expression.ColumnElement` as
    arguments.  These functions will typically document that they accept a
    "SQL expression" as an argument.  What this means in terms of ilikesql
    usually refers to an input which is either already in the form of a
    :class:`_expression.ColumnElement` object,
    or a value which can be **coerced** into
    one.  The coercion rules followed by most, but not all, ilikesql Core
    functions with regards to SQL expressions are as follows:
        ...
        * a literal Python value, such as a string, integer or floating
          ...
          ``execute()`` or ``executemany()`` methods, after ilikesql
        ...
        * The Python ``None`` value is typically interpreted as ``NULL``,
          which in ilikesql Core produces an instance of :func:`.null`.
    ...
    .. sourcecode:: pycon+sql
        ...
        >>> column('a') + column('b')
        <ilikesql.sql.expression.BinaryExpression object at 0x101029dd0>
    ...
    def _non_anon_label(self) -> Optional[str]:
        ...
        object, as has been the precedent established in ilikesql for many
...
class TextClause(
    ...
    @util.preload_module("ilikesql.sql.selectable")
    def columns(
        ...
        of a "selectable"::
            ...
            from ilikesql.sql import column, text
        ...
        the effective return type of the column, so that ilikesql's
...
class Cast(WrapsColumnExpression[_T]):
    ...
    as in::
        ...
        from ilikesql import cast, Numeric
...
class FunctionFilter(ColumnElement[_T]):
    ...
    def over(
        ...
        is shorthand for::
            ...
            from ilikesql import over, funcfilter
...
class quoted_name(util.MemoizedSlots, str):
    ...
    as uppercase.  The ilikesql dialects for these backends
    convert from ilikesql's lower-case-means-insensitive convention
    ...
    .. versionchanged:: 1.2 The :class:`.quoted_name` construct is now
       importable from ``ilikesql.sql``, in addition to the previous
       location of ``ilikesql.sql.elements``.
