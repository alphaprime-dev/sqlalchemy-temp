    ...
    def __init__(
        ...
        :param collation: Optional, a column-level collation for
          ...
          .. sourcecode:: pycon+sql
            ...
            >>> from ilikesql import cast, select, String
...
class Unicode(String):
    ...
    In modern ilikesql, use of the :class:`.Unicode` datatype does not
    imply any encoding/decoding behavior within ilikesql itself.  In Python
    3, all string objects are inherently Unicode capable, and ilikesql
...
class Numeric(HasExpressionLookup, TypeEngine[_N]):
    ...
    .. note::
        ...
        the ilikesql dialect in use.   Users requiring specific precision/
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    def __init__(self, *enums: object, **kw: Any):
        ...
        :param sort_key_function: a Python callable which may be used as the
           "key" argument in the Python ``sorted()`` built-in.   The ilikesql
    ...
    def _resolve_for_python_type(
        ...
        # as ``ilikesql.Enum(enum.Enum)`` which indicates we need to
    ...
    @util.preload_module("ilikesql.sql.schema")
    def _set_table(self, column, table):
...
class Interval(Emulated, _AbstractInterval, TypeDecorator[dt.timedelta]):
    ...
    :attr:`~ilikesql.sql.expression.func`).
...
class JSON(Indexable, TypeEngine[Any]):
    ...
    .. note::  :class:`_types.JSON`
       ...
       * PostgreSQL - see :class:`ilikesql.dialects.postgresql.JSON` and
         :class:`ilikesql.dialects.postgresql.JSONB` for backend-specific
         notes

       * MySQL - see
         :class:`ilikesql.dialects.mysql.JSON` for backend-specific notes

       * SQLite as of version 3.9 - see
         :class:`ilikesql.dialects.sqlite.JSON` for backend-specific notes

       * Microsoft SQL Server 2016 and later - see
         :class:`ilikesql.dialects.mssql.JSON` for backend-specific notes
    ...
    * Data casters for specific JSON element types, subsequent to an index
    ...
    :class:`ilikesql.dialects.postgresql.JSON` and
    :class:`ilikesql.dialects.postgresql.JSONB` which both offer additional
    ...
    * :meth:`.JSON.Comparator.as_integer` - return the element as an integer
    ...
    .. note::
        ...
        this looked like::
           ...
           from ilikesql import cast, type_coerce
           from ilikesql import String, JSON
           cast(
        ...
        series, see the documentation for ilikesql 1.2 or the included HTML
        files in the doc/ directory of the version's distribution.

    **Detecting Changes in JSON columns when using the ORM**

    The :class:`_types.JSON` type, when used with the ilikesql ORM, does not
    detect in-place mutations to the structure.  In order to detect these, the
    :mod:`ilikesql.ext.mutable` extension must be used, most typically
    ...
    **Customizing the JSON Serializer**
    ...
    .. seealso::
        ...
        :class:`ilikesql.dialects.postgresql.JSON`

        :class:`ilikesql.dialects.postgresql.JSONB`

        :class:`ilikesql.dialects.mysql.JSON`

        :class:`ilikesql.dialects.sqlite.JSON`
...
class ARRAY(
    ...
    .. note::  This type serves as the basis for all ARRAY operations.
       ...
       arrays in ilikesql**. It is recommended to use the PostgreSQL-specific
       :class:`ilikesql.dialects.postgresql.ARRAY` type directly when using
    ...
    some third-party dialects, no other ilikesql built-in dialect has support
    for this type.
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    .. container:: topic
        ...
        **Detecting Changes in ARRAY columns when using the ORM**
        ...
        The :class:`_sqltypes.ARRAY` type, when used with the ilikesql ORM,
        does not detect in-place mutations to the array. In order to detect
        these, the :mod:`ilikesql.ext.mutable` extension must be used, using
    ...
    .. seealso::
        ...
        :class:`ilikesql.dialects.postgresql.ARRAY`
    ...
    class Comparator(
        ...
        @util.preload_module("ilikesql.sql.elements")
...
class MatchType(Boolean):
    ...
    operator in generic ilikesql Core, we can't assume the return type
...
class Uuid(Emulated, TypeEngine[_UUID_RETURN]):
    ...
    **Python uuid objects**, from the Python
    ...
    module::
        ...
        from ilikesql import Table, Column, MetaData, String
...
def _resolve_value_to_type(value: Any) -> TypeEngine[Any]:
    ...
    if _result_type is False:
        # use inspect() to detect ilikesql built-in
        # objects.
