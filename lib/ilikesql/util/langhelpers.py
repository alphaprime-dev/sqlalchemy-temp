...
def decorator(target: Callable[..., Any]) -> Callable[[_Fn], _Fn]:
    ...
    def decorate(fn: _Fn) -> _Fn:
        ...
        # ilikesql that arguments should be passed positionally
...
class classproperty(property):
    ...
    :class:`~.ilikesql.ext.declarative.declared_attr`
...
_not_sa_pattern = re.compile(r"^(?:ilikesql\.(?!testing)|alembic\.)")
...
def _warnings_warn(
    ...
    # adjust the given stacklevel to be outside of ilikesql
    try:
...
_SQLA_RE = re.compile(r"ilikesql/([a-z_]+/){0,2}[a-z_]+\.py")
