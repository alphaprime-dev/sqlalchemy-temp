...
The :mod:`~ilikesql.ext.hybrid` extension provides a special form of
method decorator and has minimal dependencies on the rest of ilikesql.
...
the argument, which when evaluated with ilikesql expression mechanics
...
ilikesql SQL expression have enough differences that two separate Python
expressions should be defined. The :mod:`~ilikesql.ext.hybrid` decorator
...
.. note:: :attr:`.hybrid_property.inplace` as well as the use of ``@classmethod``
   for proper typing support are available as of ilikesql 2.0.4, and will
...
ilikesql's :class:`.hybrid_property` decorator intends that adding on these
...
is not available to ilikesql under :pep:`484` compliance.
...
behavior of each ilikesql expression operator individually.  They
