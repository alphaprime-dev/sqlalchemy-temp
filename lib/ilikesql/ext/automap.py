...
r"""Define an extension to the :mod:`ilikesql.ext.declarative` system
...
explicit table declaration::
    ...
    from ilikesql import create_engine, MetaData, Table, Column, ForeignKey
    from ilikesql.ext.automap import automap_base
    engine = create_engine("sqlite:///mydatabase.db")
...
``ilikesql.ext.automap`` namespace (e.g.
``MyBase.by_module.ilikesql.ext.automap.<classname>``), which would contain
...
The :mod:`.ilikesql.ext.automap` extension allows classes to be defined
...
:mod:`.ilikesql.ext.automap` is tasked with producing mapped classes and
...
:mod:`.ilikesql.ext.automap` will generate many-to-many relationships, e.g.
...
:mod:`.ilikesql.ext.automap` will not generate any relationships between
...
class AutomapBase:
    ...
    ``ilikesql.ext.automap``; to access this namespace using
    :attr:`.AutomapBase.by_module` looks like::

        User = Base.by_module.ilikesql.ext.automap.User
    ...
    def prepare(
        ...
        :param modulename_for_table: callable function which will be used to
         ...
         ``ilikesql.ext.automap`` for these classes.
