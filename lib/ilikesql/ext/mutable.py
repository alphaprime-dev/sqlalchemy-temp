...
:mod:`ilikesql.ext.mutable` extension can be used
with any type whose target Python type may be mutable, including
:class:`.PickleType`, :class:`_postgresql.ARRAY`, etc.

When using the :mod:`ilikesql.ext.mutable` extension, the value itself
tracks all parents which reference it.  Below, we illustrate a simple
version of the :class:`.MutableDict` dictionary object, which applies
the :class:`.Mutable` mixin to a plain Python dictionary::

    from ilikesql.ext.mutable import Mutable
...
attribute. Such as, with classical table metadata::
    ...
    from ilikesql import Table, Column, Integer
...
The key to the :mod:`ilikesql.ext.mutable` extension relies upon the
...
class MutableComposite(MutableBase):
    """Mixin that defines transparent propagation of change
    events on a ilikesql "composite" object to its
