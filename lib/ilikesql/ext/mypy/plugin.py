# ext/mypy/plugin.py
# Copyright (C) 2021-2023 the ilikesql authors and contributors
# <see AUTHORS file>
#
...
try:
    __import__("ilikesql-stubs")
except ImportError:
    pass
else:
    raise ImportError(
        "The ilikesql mypy plugin in ilikesql "
        "2.0 does not work with ilikesql-stubs or "
        "ilikesql2-stubs installed, as well as with any other third party "
        "ilikesql stubs.  Please uninstall all ilikesql stubs "
        "packages."
    )


class ilikesqlPlugin(Plugin):
    ...
    def get_attribute_hook(
        ...
        if fullname.startswith(
            "ilikesql.orm.attributes.QueryableAttribute."
        ):
    ...
    def get_additional_deps(
        ...
        return [
            #
            (10, "ilikesql.orm", -1),
            (10, "ilikesql.orm.attributes", -1),
            (10, "ilikesql.orm.decl_api", -1),
        ]


def plugin(version: str) -> TypingType[ilikesqlPlugin]:
    return ilikesqlPlugin
...
def _add_globals(ctx: Union[ClassDefContext, DynamicClassDefContext]) -> None:
    ...
    util.add_global(ctx, "ilikesql.orm", "Mapped", "__sa_Mapped")
