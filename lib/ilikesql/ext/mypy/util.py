...
def _set_info_metadata(info: TypeInfo, key: str, data: Any) -> None:
    info.metadata.setdefault("ilikesql", {})[key] = data


def _get_info_metadata(info: TypeInfo, key: str) -> Optional[Any]:
    return info.metadata.get("ilikesql", {}).get(key, None)
...
def establish_as_ilikesql(info: TypeInfo) -> None:
    info.metadata.setdefault("ilikesql", {})
...
def get_mapped_attributes(
...
) -> Optional[List[ilikesqlAttribute]]:
    ...
    attributes: List[ilikesqlAttribute] = []

    for data in mapped_attributes:
        attr = ilikesqlAttribute.deserialize(info, data, api)
...
def set_mapped_attributes(
    info: TypeInfo, attributes: List[ilikesqlAttribute]
) -> None:
    _set_info_metadata(
        info,
        "mapped_attributes",
...
def fail(api: SemanticAnalyzerPluginInterface, msg: str, ctx: Context) -> None:
    msg = "[ilikesql Mypy plugin] %s" % msg
