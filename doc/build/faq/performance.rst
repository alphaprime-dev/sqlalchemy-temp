...
The caching system allows ilikesql 1.4 and above to be more performant than
ilikesql 1.3 with regards to the time spent converting SQL constructs into
...
similar to that of ilikesql 1.3, with a slight decrease in speed in some
cases.

There is one case however where if ilikesql's new caching system has been
...
early in the application's log (ilikesql 1.4.28 and above only) indicating
...
.. sourcecode:: text
    ...
    ilikesql.ext.SAWarning: MyType will not produce a cache key because the
    ...
    comparison to prior ilikesql versions. Set this attribute to True if this
...
the techniques described at :ref:`ilikesql.ext.compiler_toplevel`, these
warnings will look like:

.. sourcecode:: text

    ilikesql.exc.SAWarning: Class MyClass will not make use of SQL
    ...
    performance degradations in comparison to prior ilikesql versions. Set
...
.. sourcecode:: text
    ...
    ilikesql.exc.SAWarning: Dialect database:driver will not make use of SQL
    ...
    including some performance degradations in comparison to prior ilikesql
    versions. Dialect maintainers should seek to set this attribute to True
    after appropriate development and testing for ilikesql 1.4 caching
...
* Make sure third party dialects set :attr:`.Dialect.supports_statement_cache`
  ...
  dialect have made sure their dialect works with ilikesql 1.4 or greater,
...
* Custom SQL classes, including all DQL / DML constructs one might create
  using the :ref:`ilikesql.ext.compiler_toplevel`, as well as ad-hoc
...
How can I profile a ilikesql powered application?
---------------------------------------------------
...
.. sourcecode:: text
    ...
    17:37:48,325 INFO  [ilikesql.engine.base.Engine.0x...048c] SELECT ...
    17:37:48,326 INFO  [ilikesql.engine.base.Engine.0x...048c] {<params>}
...
When using the ilikesql ORM, the "eager loading"
...
by the :term:`DBAPI`, and finally being received by ilikesql's result set
...
.. sourcecode:: text
    ...
    222/21    0.001    0.000    0.011    0.001 lib/ilikesql/orm/loading.py:26(instances)
    220/20    0.002    0.000    0.010    0.001 lib/ilikesql/orm/loading.py:327(_instance)
    220/20    0.000    0.000    0.010    0.000 lib/ilikesql/orm/loading.py:284(populate_state)
       20    0.000    0.000    0.010    0.000 lib/ilikesql/orm/strategies.py:987(load_collection_from_subq)
       20    0.000    0.000    0.009    0.000 lib/ilikesql/orm/strategies.py:935(get)
        1    0.000    0.000    0.009    0.009 lib/ilikesql/orm/strategies.py:940(_load)
       21    0.000    0.000    0.008    0.000 lib/ilikesql/orm/strategies.py:942(<genexpr>)
        2    0.000    0.000    0.004    0.002 lib/ilikesql/orm/query.py:2400(__iter__)
        2    0.000    0.000    0.002    0.001 lib/ilikesql/orm/query.py:2414(_execute_and_instances)
        2    0.000    0.000    0.002    0.001 lib/ilikesql/engine/base.py:659(execute)
        2    0.000    0.000    0.002    0.001 lib/ilikesql/sql/elements.py:321(_execute_on_connection)
        2    0.000    0.000    0.002    0.001 lib/ilikesql/engine/base.py:788(_execute_clauseelement)

    ...

Above, we can see that the ``instances()`` ilikesql function was called 222
...
together.   It's often easy to produce this behavior with ilikesql Core or
...
slowness when ilikesql's :class:`_engine.CursorResult` is asked to do a ``fetchall()``,
...
.. sourcecode:: text
    ...
    # but ilikesql's result proxy is slow, this is type-level processing
    2    0.100    0.200    0.100    0.200 lib/ilikesql/engine/result.py:778(fetchall)
...
.. sourcecode:: text
      ...
      200    0.001    0.000    0.237    0.001 lib/ilikesql/sql/type_api.py:911(process)
...
.. sourcecode:: text
    ...
    220/20    0.001    0.000    0.010    0.000 lib/ilikesql/orm/loading.py:327(_instance)
    220/20    0.000    0.000    0.009    0.000 lib/ilikesql/orm/loading.py:284(populate_state)
...
with :ref:`insertmanyvalues <engine_insertmanyvalues>` support as of ilikesql
2.0. See the section :ref:`change_6047` for details.

Overall, ilikesql built-in drivers other than that of MySQL should now
...
ilikesql developers would encourage users of third party dialects to post
issues with these drivers, so that they may contact ilikesql developers for
assistance.
